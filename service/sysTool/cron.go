...
func (cs *CronService) GetCronList(pageInfo commonReq.PageInfo) (cronModelList []modelSysTool.CronModel, total int64, err error) {
	db := global.Db.Model(&modelSysTool.CronModel{})
...
func (cs *CronService) AddCron(cronModel *modelSysTool.CronModel) (*modelSysTool.CronModel, error) {
	...
	err := global.Db.Create(cronModel).Error
...
func (cs *CronService) DeleteCron(id uint) error {
	...
	if errors.Is(global.Db.Where("id = ?", id).First(&cronModel).Error, gorm.ErrRecordNotFound) {
		return errors.New("记录未找到")
	}
	// 删除定时任务
	global.Cron.Remove(cron.EntryID(cronModel.EntryId))
	// 删除数据库记录
	return global.Db.Unscoped().Delete(&cronModel).Error
}

// DeleteCronByIds 批量删除cron
func (cs *CronService) DeleteCronByIds(ids []uint) error {
	var cronModels []modelSysTool.CronModel
	global.Db.Find(&cronModels, ids)
	// 删除定时任务
	for _, value := range cronModels {
		global.Cron.Remove(cron.EntryID(value.EntryId))
	}
	// 删除数据库记录
	return global.Db.Unscoped().Delete(&cronModels).Error
}

// EditCron 编辑cron
func (cs *CronService) EditCron(instance *modelSysTool.CronModel) (*modelSysTool.CronModel, error) {
	if errors.Is(global.Db.Where("id = ?", instance.ID).First(&modelSysTool.CronModel{}).Error, gorm.ErrRecordNotFound) {
	...
	if instance.Open {
		if !utils.IsContain(utils.GetEntries(), instance.EntryId) {
			entryId, err := global.Cron.AddJob(instance.Expression, instance)
			if err != nil {
				return nil, err
			} else {
				instance.Open = true
		...
		if instance.EntryId != 0 {
			global.Cron.Remove(cron.EntryID(instance.EntryId))
			instance.EntryId = 0
		}
		instance.Open = false
	}
	err := global.Db.Omit("created_at").Save(instance).Error
...
func (cs *CronService) SwitchOpen(id uint, open bool) (resId int, err error) {
	...
	if open && !utils.IsContain(utils.GetEntries(), cronModel.EntryId) {
		...
		if err != nil {
			...
			err = global.Db.Model(&cronModel).Updates(map[string]interface{}{"open": true, "entryId": entryId}).Error
		}
		resId = int(entryId)
	} else {
		...
		err = global.Db.Model(&cronModel).Updates(map[string]interface{}{"open": false, "entryId": 0}).Error
		resId = 0
	}

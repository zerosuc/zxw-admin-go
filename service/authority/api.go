...
func (a *ApiService) AddApi(api *modelAuthority.ApiModel) (*modelAuthority.ApiModel, error) {
	if !errors.Is(global.Db.Where("path = ? AND method = ?", api.Path, api.Method).First(&modelAuthority.ApiModel{}).Error, gorm.ErrRecordNotFound) {
...
func (a *ApiService) GetApis(apiSp authorityReq.ApiSearchParams) ([]modelAuthority.ApiModel, int64, error) {
	...
	db := global.Db.Model(&modelAuthority.ApiModel{})
...
func (a *ApiService) GetElTreeApis(roleId uint) (list []modelAuthority.ApiTree, checkedKey []string, err error) {
	...
	var apiGroup []string
	err = global.Db.Model(&modelAuthority.ApiModel{}).Distinct().Pluck("api_group", &apiGroup).Error
	if err != nil {
...
func (a *ApiService) DeleteApi(id uint) (err error) {
	...
	if errors.Is(global.Db.Where("id = ?", id).First(&apiModel).Error, gorm.ErrRecordNotFound) {
...
func (a *ApiService) DeleteApiById(ids []uint) (err error) {
	...
	err = global.Db.Find(&apis, "id in ?", ids).Unscoped().Delete(&apis).Error
	// 删除对应casbin条目
	if err == nil {
		for _, sysApi := range apis {
			ok := serviceBase.CasbinServiceApp.ClearCasbin(1, sysApi.Path, sysApi.Method)
			if !ok {
				global.Log.Error(fmt.Sprintf("%s:%s casbin同步清理失败", sysApi.Path, sysApi.Method))
			}
		}
	}
...
func (a *ApiService) EditApi(instance *modelAuthority.ApiModel) (err error) {
	var apiModel modelAuthority.ApiModel
	if errors.Is(global.Db.Where("id = ?", instance.ID).First(&apiModel).Error, gorm.ErrRecordNotFound) {
		return errors.New("记录不存在")
	}

	if apiModel.Path != instance.Path || apiModel.Method != instance.Method {
		if !errors.Is(global.Db.Where("path = ? AND method = ?", instance.Path, instance.Method).First(&modelAuthority.ApiModel{}).Error, gorm.ErrRecordNotFound) {
	...
	return global.Db.Omit("created_at").Save(instance).Error
}

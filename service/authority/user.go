...
func (us *UserService) GetUsers(pageInfo commonReq.PageInfo) ([]authorityRes.UserResult, int64, error) {
	...
	db := global.Db.Model(&modelAuthority.UserModel{})
...
func (us *UserService) DeleteUser(id uint) (err error) {
	return global.Db.Where("id = ?", id).Unscoped().Delete(&modelAuthority.UserModel{}).Error
}

// AddUser 添加用户
func (us *UserService) AddUser(instance *authorityReq.AddUser) (err error) {
	if errors.Is(global.Db.Where("id = ?", instance.RoleModelID).First(&modelAuthority.RoleModel{}).Error, gorm.ErrRecordNotFound) {
...
func (us *UserService) ModifyPass(mp *authorityReq.ModifyPass) (err error) {
	var userModel modelAuthority.UserModel
	if errors.Is(global.Db.Where("id = ? and password = ?", mp.ID, utils.MD5V([]byte(mp.OldPassword))).First(&userModel).Error, gorm.ErrRecordNotFound) {
...
func (us *UserService) SwitchActive(sa *authorityReq.SwitchActive) (err error) {
	var userModel modelAuthority.UserModel
	if errors.Is(global.Db.Where("id = ?", sa.ID).First(&userModel).Error, gorm.ErrRecordNotFound) {
		return errors.New("记录不存在")
	}

	return global.Db.Model(&userModel).Update("active", sa.Active).Error
}

...
func (fs *FileService) Upload(file *multipart.FileHeader) (*modelFileM.FileModel, error) {
	...
	fullPath := fmt.Sprintf("%s/%s", global.Config.File.Upload, transformName)
...
func (fs *FileService) GetFileList(params fileMReq.FileSearchParams) ([]modelFileM.FileModel, int64, error) {
	...
	db := global.Db.Model(&modelFileM.FileModel{})
...
func (fs *FileService) Delete(fileName string) (err error) {
	// 物理删除
	err = os.Remove(fmt.Sprintf("%s/%s", global.Config.File.Upload, fileName))

	// 删除数据库记录
	if err == nil {
		err = global.Db.Where("file_name = ?", fileName).Delete(&modelFileM.FileModel{}).Error
	}
